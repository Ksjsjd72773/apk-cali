    ...
    def on_equals(self, instance):
        ...
        try:
            ...
            if isinstance(result, float) and result == int(result):
                result = int(result)
            self.lbl_expression.text = full_expr.replace("*", "×").replace("/", "÷") + " ="
            self.lbl_result.text = str(result)
            self.current = str(result)
            self.expression = ""
        except ZeroDivisionError:
            self.lbl_result.text = "خطأ: قسمة على صفر"
            self.current = ""
            self.expression = ""
        except Exception:
            self.lbl_result.text = "خطأ"
            self.current = ""
            self.expression = ""
